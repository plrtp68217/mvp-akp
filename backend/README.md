...
В модели order находятся переменные total, status, items_count
Далее в regitsry они зачем то захардкожены
Другие переменные находятся в order.metadata_json...
У custom_variable есть какой-то source_path, 

Сейчас все это выглядит максимально запутанно и перегруженно. Необходимо стандартизировать
все переменные и хранить их в одном месте.